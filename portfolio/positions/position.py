...
from portfolio.positions.entry import Entry
...
class Position:
    ...
    def __init__(self, data: FeaturesData):
        """
        Initialize the Position object with the features data.
        :param data: FeaturesData object containing equity and market data
        """
        self.data = data
        self.entries = {}

    def rebalance(self, value: float, date: datetime, slippage: float) -> float:
        """
    ...
    def _trade(self, date: datetime, shares: int, slippage: float) -> float:
        """
        ...
        if shares != 0:
            delta += self._open(date, shares, slippage)
    ...
    def _open(self, date: datetime, shares: int, slippage: float):
        """
        ...
        """
        entry = Entry(self.data, shares, date, slippage)
