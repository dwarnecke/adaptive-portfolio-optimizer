...
class Covariance:
    ...
    def __init__(
        ...
        """
        Initialize the Covariance object with a dataset and shrinkage factor.
        :param dataset: FeaturesDataset containing equity data
        :param parameters: Dictionary of portfolio parameters, default config
        """
        # Alpha must be between 0 and 1
        if not (0 <= parameters["covariance_shrinkage"] <= 1):
    ...
    def _calc_log_daily_returns(self) -> DataFrame:
        ...
        for index in self.dataset.index:
            features_data = self.dataset.data[index]
            log_returns = features_data.get_series("Log Close").diff()
